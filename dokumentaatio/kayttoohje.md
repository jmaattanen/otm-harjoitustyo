...
Sovelluksen [Release 3](https://github.com/jmaattanen/otm-harjoitustyo/releases/tag/loppupalautus)-version pystyy suorittamaan koneella, jolle on asennettu Java versio 1.8.
Ohjelman voi suorittaa polusta, jonne jar-tiedosto on kopioitu, komennolla

```
java -jar TranscribeBuddy-3.0.jar
```
...
[Postgresin sivuilta](https://jdbc.postgresql.org/documentation/head/connect.html) lˆytyy lis‰‰ ohjeita tietokantaan yhdist‰misest‰.

Sovellus tukee myˆs SQLite-tietokantaa. Seuraava konfiguraatio luo _munbeissi.db_-nimisen tietokannan sovelluksen juureen:

```
databaseURL=jdbc:sqlite:munbeissi.db
```

Sovelluksella luotujen dokumenttien tallennuskansion voi m‰‰ritt‰‰ seuraavasti:

```
documentHome=/oletus/polku/asiakirjojen/tallennukseen/
```

Muutokset tiedostoon _config.properties_ vaativat sovelluksen uudelleenk‰ynnist‰mist‰.
...
| _ctrl_ + _shift_ + _space_ | Aloita ‰‰niraidan toisto asetetusta merkist‰ rivin‰kym‰ss‰    |
